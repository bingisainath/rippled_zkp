// Poseidon parameters for BN-128 (the alt_bn128 scalar field), t=3, x^5
// S-box. Constants come from iden3 circomlibjs, the de-facto reference for
// Circom and Tornado Cash, originally generated by the Sage script
// generate_parameters_grain.sage from
//   https://extgit.iaik.tugraz.at/krypto/hadeshash
// Reference: Grassi et al., "Poseidon: A New Hash Function for
// Zero-Knowledge Proof Systems", USENIX Security 2021, eprint 2019/458.
//
// Layout for t=3: every round — full or partial — adds an ARC constant to
// all t state elements, so the table holds (R_F + R_P) * t = (8 + 57) * 3
// = 195 entries in a flat array indexed by [round * t + i]. The MDS matrix
// is a fixed t x t = 9 entries.
//
// Constants are stored as decimal strings to avoid endianness ambiguity
// when converted to field elements at static-init time.
//
...
// the reference-vector test. The full 195-entry table is loaded
// from poseidon_constants.inc which contains the Sage-generated values.
//
// SANITY ANCHOR (must match exactly).
...
// REFERENCE VECTORS
// These three pairs anchor the unit tests. Any implementation that
